...
from .service import exportar_dados_usuario, excluir_conta_usuario, consultar_consentimento
...
@router.get(
    "/consentimento",
    summary="Consultar status de consentimento LGPD",
    description=(
        "Retorna o status atual do consentimento de termos de uso do usuário autenticado, "
        "incluindo a data em que o consentimento foi registrado (Art. 18, VII LGPD)."
    ),
)
async def consentimento(
    ...
    usuario_id = str(usuario["_id"])
    return await consultar_consentimento(usuario_id, db)
