...
async def obter_checklist(edital_id: str, usuario: dict, db: AsyncIOMotorDatabase) -> dict:
    ...
    items = [
        {
            **r,
            "plano_necessario": r.get("plano", "free"),
            "concluido": r["id"] in concluidos_ids,
            "documento_id": documentos_map.get(r["id"]),
        }
        for r in requisitos
    ]
    total = len(items)
    n_concluidos = sum(1 for i in items if i["concluido"])

    return {
        "edital_id": edital_id,
        "progresso": round((n_concluidos / total) * 100, 1) if total > 0 else 0.0,
        "items": items,
    }
...
async def marcar_requisito(
    ...
    item_existente = next((i for i in itens if i["requisito_id"] == item_id), None)

    if item_existente:
        item_existente["concluido"] = concluido
        item_existente["documento_id"] = documento_id
    else:
        itens.append({"requisito_id": item_id, "concluido": concluido, "documento_id": documento_id})
